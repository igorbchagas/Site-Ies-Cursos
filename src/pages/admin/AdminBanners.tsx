...
    
    try {
      // 1. Chama o serviço apenas com o ID. O serviço faz a leitura, inversão e escrita.
      // 2. O serviço retorna o NOVO status booleano (true ou false)
      const newStatus = await bannerService.toggleActive(banner.id);
      
      // 3. ATUALIZA o estado LOCAL usando o valor CONFIÁVEL retornado do backend
...
      
      // NOTA: A persistência no Hero é tratada no front-end público, veja a seção 2.
    } catch (error) {
...
             Banners de promoções ou avisos ({banners.length}/5)
           </h3>
           <p className="text-xs text-zinc-400 mt-1 flex items-center gap-1">
             <Zap size={12} className="text-orange-400" />
             Arraste os itens na lista abaixo para definir a ordem de exibição.
           </p>
        </div>

        <label className={`
          flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-all shadow-md 
...
            {uploading ? 'Enviando...' : banners.length >= 5 ? 'Limite Atingido' : 'Adicionar Banner'}
          </span>
          <input 
            type="file" 
...
                value={banner}
                layout 
                // CRÍTICO: dragElastic cria a "mola" que impede o arrasto excessivo
                dragElastic={0.01} 
                className={`
...
                    <div className='text-zinc-500 opacity-70 cursor-move'>
                        <GripVertical size={20} />
                    </div>

                    <div className="w-20 h-12 flex-shrink-0 rounded overflow-hidden bg-zinc-800 relative">
                        <img 
...
                            </span>
                            <span className={`w-2 h-2 rounded-full ${banner.ativo ? 'bg-green-500' : 'bg-zinc-700'}`}></span>
                        </div>
                    </div>
                </div>
...
                        className="p-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg text-white transition-colors border border-zinc-600 shadow-sm"
...
                        className="p-2 bg-red-500/20 hover:bg-red-600 rounded-lg text-red-200 hover:text-white transition-colors border border-red-500/50 shadow-sm"
                        title="Excluir"
                    >
                        <Trash2 size={18} />
                    </button>
